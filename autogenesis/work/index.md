# Work

Scoped under **autogenesis/work/**.

- [2026-09-12-catalog-think-integration](2026-09-12-catalog-think-integration.md) — **designed**; implementation blocked pending Autogenesis design approval
- [Dogfood the SOLID lens on Autogenesis](2026-09-12-14-solid-dogfood-autogenesis.md) - **done locally; implement PR pending**
- [2026-09-12-explicit-discuss-integration](2026-09-12-explicit-discuss-integration.md) — **done**
- [SOLID-derived skill design lens](2026-09-12-solid-skill-design-lens.md) - **done locally; lens PR still open**
- [Remove Construct from the Autogenesis runtime contract](2026-09-11-remove-construct-binding.md) - **done locally**
- [Instruction-first modules for derived skills](2026-09-11-instruction-first-skill-modules.md) - **done locally; final CI remains**
- [S8 core and Autogenesis profile self-application](2026-09-11-s8-core-profile-self-application.md) - **deferred; replaced by instruction-first design**
- [Parent-routed Skill Module pattern](2026-09-11-parent-routed-skill-module-pattern.md) - **implemented locally; final CI and live acceptance pending**
- [Skill modules and invocation discipline](2026-09-11-skill-module-invocation.md) - **implemented locally; final CI and live behaviour acceptance pending**
- [Approved execution plan and model delegation](2026-09-11-skill-module-invocation-execution.md)

- [2026-09-05-atlas-storage-semantics](2026-09-05-atlas-storage-semantics.md) — **done locally**
- [2026-08-26-residuals-vs-protostar](2026-08-26-residuals-vs-protostar.md) — **done**
- [2026-08-26-discuss-skill-formalization](2026-08-26-discuss-skill-formalization.md) — **done**
- [2026-08-26-medium-atlas-first-creative-loop](2026-08-26-medium-atlas-first-creative-loop.md) — **done** (subject medium; plan lives in medium Atlas)
- [2026-08-26-gamma-atlas-first-creative-loop](2026-08-26-gamma-atlas-first-creative-loop.md) — **done** (subject gamma; plan lives in gamma Atlas)

- [activation card to peers](activation-card-to-peers.md) — **done**
- [agent brain session 2026 08 21 close](agent-brain-session-2026-08-21-close.md) — **done**
- [autogenesis activation card extend](autogenesis-activation-card-extend.md) — **done**
- [autogenesis catalogue review gate v1](autogenesis-catalogue-review-gate-v1.md) — **done**
- [autogenesis challenge adversarial construct](autogenesis-challenge-adversarial-construct.md) — **done**
- [Migrate autogenesis skill process memory from okf-wiki to Atlas](autogenesis-okf-wiki-to-atlas-migration-v1.md) — **done**
- [Path atlas-migrate — Autogenesis discipline extending atlas migrate](autogenesis-path-atlas-migrate-v1.md) — **done**
- [autogenesis patterns as genesis extension v1](autogenesis-patterns-as-genesis-extension-v1.md) — **done**
- [autogenesis patterns module v1](autogenesis-patterns-module-v1.md) — **done**
- [Plan home = subject Atlas; initiate/migrate offers when missing](autogenesis-plan-home-subject-atlas-v1.md) — **done**
- [autogenesis remember at path end](autogenesis-remember-at-path-end.md) — **done**
- [autogenesis work id lineage](autogenesis-work-id-lineage.md) — **done**
- [okf wiki ontology type v1](okf-wiki-ontology-type-v1.md) — **done**
- [Align root think-* skills to Atlas substrate](think-skills-atlas-align-v1.md) — **done**
- [Add thorough relationship-review + quality relates_to gate to atlas-migrate](atlas-migrate-relationship-review-gate-v1.md) — **done**
- [post-migrate-cleanup-v1](post-migrate-cleanup-v1.md) — done
- [2026-08-24-work-id-date-prefix](2026-08-24-work-id-date-prefix.md) — **done**
- [2026-08-26-discuss-mvp](2026-08-26-discuss-mvp.md) — **designed** (subject discuss; plan lives in discuss Atlas)
- [2026-08-26-discuss-sprout-leaves](2026-08-26-discuss-sprout-leaves.md) — **designed** (subject discuss)
- [2026-08-26-autogenesis-discuss-activation](2026-08-26-autogenesis-discuss-activation.md) — **done** (path discuss, v0.3.11)
- [2026-08-26-discuss-from-active-design](2026-08-26-discuss-from-active-design.md) — **done** (v0.3.12)
- [atlas-migrate-cli-improve-v1](atlas-migrate-cli-improve-v1.md) — **designed**
